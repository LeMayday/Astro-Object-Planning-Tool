...
def master_flat(input_dir, output_dir, bias_dir):
    ...
    siril.cd(output_dir)
    siril.calibrate('flat', bias=bias_dir + '/bias_stacked')
    siril.stack('pp_flat', type='rej', sigma_low=3, sigma_high=3, norm='mul')
    
def calibrate_lights(input_dir, output_dir, bias_dir):
    siril.cd(input_dir)
    siril.convert('light', out=output_dir, fitseq=True)
    siril.cd(output_dir)
    siril.calibrate('light', bias=bias_dir + '/bias_stacked', flat='pp_flat_stacked',
